...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train']:
            ...
            best_model_wts = copy.deepcopy(model.state_dict())
            torch.save(model_ft.state_dict(),'./resnet101_model1fc_trial'+str(trial)+'.pt')
        print()
...
optimizer_ft = optim.SGD([{'params':model_ft.features.parameters()},{'params': model_ft.fc1.parameters(),'lr':0.1}],lr=0.01, momentum=0.9, weight_decay=1e-4)
...
exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, gamma=0.1,step_size=5)
