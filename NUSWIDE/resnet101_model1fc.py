...
#                   for x in ['train', 'val']}
# =============================================================================
image_datasets = {x: CocoDataset('./'+x+'_annotation.csv',
                                 './ImageData/Flickr',
                                 data_transforms[x])
                  for x in ['train', 'val']}
...
image_datasets_test = CocoDataset('./test_annotation.csv', './ImageData/Flickr',data_transforms['test'])
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train']:
            ...
            best_model_wts = copy.deepcopy(model.state_dict())
            torch.save(model_ft.state_dict(),'./resnet101_model1fc_trial'+str(trial)+'imgSize'+str(img_size)+'.pt')
        print()
...
#model_ft.layer6 = nn.Conv2d(128, num_classes, kernel_size=1, stride=1)
...
optimizer_ft = optim.SGD([{'params':model_ft.features.parameters()},{'params': model_ft.fc1.parameters(),'lr':0.1}],lr=0.01, momentum=0.9)
...
exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, gamma=0.1,step_size=6)
...
#
if trial>10:
    model_ft.load_state_dict(torch.load('./results/resnet101_model1fc_trial0.pt'))
    model_ft = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler,num_epochs=20)
else:
    model_ft = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler,num_epochs=20)
torch.save(model_ft.state_dict(),'./results/resnet101_model1fc_trial'+str(trial)+'imgSize'+str(img_size)+'.pt')
